...
func (t *TableTestSuite) SetupSuite() {
	...
	t.Require().NoError(bucket.Upload(context.Background(), "test/location/uuid.metadata.json", bytes.NewReader([]byte(ExampleTableMetadataV2))))

	tbl, err := table.NewFromLocation([]string{"foo"}, "test/location/uuid.metadata.json", bucket)
	t.Require().NoError(err)
	t.Require().NotNil(tbl)

	t.Equal([]string{"foo"}, tbl.Identifier())
	t.Equal("test/location/uuid.metadata.json", tbl.MetadataLocation())
	t.Equal(bucket, tbl.FS())
...
func (t *TableTestSuite) TestNewTableFromReadFile() {
	...
	tbl2, err := table.NewFromLocation([]string{"foo"}, "test/location/uuid.metadata.json", bucket)
	t.Require().NoError(err)
