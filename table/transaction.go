...
// UpdateSpec is used to update the partition spec of a table as part of the transaction.
//
// Parameters:
...
// AddFiles adds file paths to the current snapshots.
//
// AddFiles can work with Parquet files both with and without field IDs in their metadata:
//   - Files with field IDs: When field IDs are present in the Parquet metadata, they must match the corresponding field IDs
//     in the Iceberg table schema. This is common for files generated by tools like Spark or when using or other libraries
//     with explicit field ID metadata.
//   - Files without field IDs: When field IDs are absent, the table must have a Name Mapping to map field names to Iceberg
//     field IDs. add_files will automatically create a Name Mapping based on the table's current schema if one doesn't already exist.
//
// AddFiles only requires the client to read the existing parquet files' metadata footer to infer the partition value of each file.
// This implementation also supports adding files to Iceberg tables with partition transforms like MonthTransform, and TruncateTransform
// which preserve the order of the values after the transformation (Any Transform that has the preserves_order property set to True is supported).
// Please note that if the column statistics of the PartitionField's source column are not present in the parquet metadata, the partition value is
// inferred as None.
...
func (t *Transaction) AddDataFiles(ctx context.Context, filesToAdd []iceberg.DataFile, snapshotProps iceberg.Properties) error {
	fs, err := t.tbl.fsF(ctx)
	if err != nil {
		return err
	}
