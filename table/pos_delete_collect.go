		...
		for e, err := range m.Entries(fs, true) {
			...
			if seq := e.SequenceNum(); seq >= 0 {
				...
				candidates = append(candidates, positionDeleteCandidate{
					df:           df,
					partitionKey: positionDeletePartitionKey(df.SpecID(), df.Partition()),
					seq:          seq,
				})
			}
		}
...
func minSurvivorSeqByPartition(ctx context.Context, fs iceio.IO, manifests []iceberg.ManifestFile, rewrittenPaths map[string]struct{}) (map[string]int64, error) {
	...
	for _, m := range manifests {
		...
		for e, err := range m.Entries(fs, true) {
			...
			key := positionDeletePartitionKey(df.SpecID(), df.Partition())
...
func positionDeletePartitionKey(specID int32, part map[int]any) string {
	key := fmt.Appendf(nil, "%d:", specID)
	...
	slices.Sort(ids)
	for _, id := range ids {
		key = fmt.Appendf(key, "%d=%v;", id, part[id])
	}
