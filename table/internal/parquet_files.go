...
func (pfs *ParquetFileSource) GetReader(ctx context.Context) (result FileReader, err error) {
	...
	//
	// Extension-typed columns are skipped. An extension type declares its own
	// storage type, and pqarrow's applyOriginalStorageMetadata rejects the file
	// outright ("mismatch storage type 'large_binary' for extension type ...")
	// when the inferred storage no longer equals the declared one. Geometry and
	// geography columns, whose ProjJSON CRS round-trips through an extension
	// type over binary storage, are the concrete case. Widening them is not
	// worth failing the read for: they are bounded by the geometry encoding,
	// not by user-supplied blob payloads.
	forceable, err := forceLargeColumns(rdr, &arrProps)
	if err != nil {
		return nil, err
	}
	for _, i := range forceable {
...
// forceLargeColumns returns the physical (leaf) column indices that may safely
// be widened to 64-bit offsets: every leaf except those sitting under an Arrow
// extension type, which declares its own storage type and must keep it.
//
// It costs one extra schema-manifest build per file open — not per batch. The
// manifest is what resolves the file's ARROW:schema key-value metadata into
// concrete Arrow types, and that resolution is the only place extension types
// become visible. props must not have any force-large index set yet, so the
// manifest reflects the file's natural types.
func forceLargeColumns(rdr *file.Reader, props *pqarrow.ArrowReadProperties) ([]int, error) {
	fileMeta := rdr.MetaData()
	manifest, err := pqarrow.NewSchemaManifest(fileMeta.Schema, fileMeta.KeyValueMetadata(), props)
	if err != nil {
		return nil, err
	}

	numCols := fileMeta.Schema.NumColumns()
	cols := make([]int, 0, numCols)
	for i := range numCols {
		// A leaf with no Arrow field mapping is not something pqarrow would
		// widen in the first place.
		if field, ok := manifest.ColIndexToField[i]; ok && !underExtensionType(manifest, field) {
			cols = append(cols, i)
		}
	}
...
// underExtensionType reports whether field or any of its ancestors carries an
// Arrow extension type. An extension may wrap a nested type, so a binary leaf
// several levels down still feeds its declared storage type.
func underExtensionType(manifest *pqarrow.SchemaManifest, field *pqarrow.SchemaField) bool {
	for f := field; f != nil; f = manifest.ChildToParent[f] {
		if f.Field != nil && f.Field.Type != nil && f.Field.Type.ID() == arrow.EXTENSION {
			return true
		}
	}
