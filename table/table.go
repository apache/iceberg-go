...
func (t Table) FS() objstore.Bucket      { return t.fs }
...
func New(ident Identifier, meta Metadata, location string, fs objstore.Bucket) *Table {
	return &Table{
		identifier:       ident,
		metadata:         meta,
...
func NewFromLocation(ident Identifier, metalocation string, bucket objstore.Bucket) (*Table, error) {
	...
	rdr, err := bucket.Get(context.TODO(), metalocation)
	if err != nil {
		return nil, err
	}
	defer rdr.Close()

	if meta, err = ParseMetadata(rdr); err != nil {
		return nil, err
	}
