...
func TestPartitionResidualPreservesArrowNullFiltering(t *testing.T) {
	checkPartitionResidualArrowFiltering(t, iceberg.PrimitiveTypes.Int32, nil, "null",
		iceberg.Int32Literal(5), []iceberg.Transform{iceberg.IdentityTransform{}})
}
			...
			for _, predicate := range predicates {
				for _, filter := range []iceberg.BooleanExpression{
					...
					t.Run(filter.String(), func(t *testing.T) {
						...
						require.NoError(t, err)
						residualPlan := newPartitionResidualPlan(schema, &spec, bound, true)
						residual := bound
						if residualPlan != nil {
							candidate, changed := residualPlan.residual(map[int]any{1000: value})
							if changed {
								residual = candidate
							}
						}
