...
func NewEqualityDeleteWriter(
	...
	return &EqualityDeleteWriter{
		...
		schema:         createEqualityDeleteSchema(equalityFields, tableSchema),
...
// createEqualityDeleteSchema creates a schema for equality delete files.
// It includes a file_path field and the specified equality fields from the table schema.
func createEqualityDeleteSchema(equalityFields []int, tableSchema *iceberg.Schema) *iceberg.Schema {
	fields := make([]iceberg.NestedField, 0, len(equalityFields)+1)
	
	// Add file_path field as the first field
	fields = append(fields, iceberg.NestedField{
		ID:       2147483546, // Standard field ID for file_path in delete files
		Name:     "file_path",
	...
	for _, fieldID := range equalityFields {
...
// PositionDeleteWriter writes position delete files.
...
// NewPositionDeleteWriter creates a new position delete writer.
func NewPositionDeleteWriter(
	...
	// Position delete schema has file_path (string) and pos (long) fields
	// Create a simple schema for position deletes with file_path and pos fields
	posDeleteSchema := iceberg.NewSchema(0,
		iceberg.NestedField{ID: 2147483546, Name: "file_path", Type: iceberg.PrimitiveTypes.String, Required: true},
		iceberg.NestedField{ID: 2147483545, Name: "pos", Type: iceberg.PrimitiveTypes.Int64, Required: true},
	)
	...
	return &PositionDeleteWriter{
		...
		writeProps:     internal.GetFileFormat(iceberg.ParquetFile).GetWriteProperties(writeProps),
...
// Write adds a position delete to the writer.
func (w *PositionDeleteWriter) Write(ctx context.Context, delete interface{}) error {
	pos, ok := delete.(*PositionDelete)
	if !ok {
		return fmt.Errorf("expected *PositionDelete, got %T", delete)
	}
	
	// Convert position delete to arrow record
	record, err := w.positionDeleteToRecord(pos)
	if err != nil {
		return fmt.Errorf("converting position delete to record: %w", err)
	}
...
// Flush writes the current batch to a delete file.
func (w *PositionDeleteWriter) Flush(ctx context.Context) ([]iceberg.DataFile, error) {
	if len(w.currentBatch) == 0 {
		return nil, nil
	}
	...
	if err != nil {
		return nil, fmt.Errorf("writing position delete file: %w", err)
	}
	
	// Convert to position delete file
	posDeleteFile := &positionDataFile{
		DataFile: dataFile,
	}
	
	w.writtenFiles = append(w.writtenFiles, posDeleteFile)
	...
	return []iceberg.DataFile{posDeleteFile}, nil
}

// GetResult returns all written delete files.
func (w *PositionDeleteWriter) GetResult() []iceberg.DataFile {
	return w.writtenFiles
}

// Close flushes any remaining data and closes the writer.
func (w *PositionDeleteWriter) Close() error {
	// Release any remaining records in the current batch
	for _, record := range w.currentBatch {
		record.Release()
	}
...
// positionDeleteToRecord converts a PositionDelete to an Arrow record.
func (w *PositionDeleteWriter) positionDeleteToRecord(pos *PositionDelete) (arrow.Record, error) {
	fields := w.schema.Fields()
	builders := make([]array.Builder, len(fields))
	
	// Create builders for each field (file_path and pos)
	for i, field := range fields {
		arrowType, err := table.TypeToArrowType(field.Type, false, false)
		if err != nil {
			return nil, fmt.Errorf("converting field type to arrow: %w", err)
	...
	// Add file_path (first field)
	builders[0].(*array.StringBuilder).Append(pos.FilePath)
	
	// Add position (second field)
	builders[1].(*array.Int64Builder).Append(pos.Position)
...
// positionDataFile wraps a DataFile to indicate it's a position delete file.
type positionDataFile struct {
	iceberg.DataFile
}

// ContentType returns that this is a position delete file.
func (p *positionDataFile) ContentType() iceberg.ManifestEntryContent {
	return iceberg.EntryContentPosDeletes
}
