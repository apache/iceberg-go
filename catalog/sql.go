...
const (
	SqlDialectKey        = "sql.dialect"
	SqlDriverKey         = "sql.driver"
	initCatalogTablesKey = "init_catalog_tables"
)

func init() {
	Register("sql", RegistrarFunc(func(uri string, p iceberg.Properties) (c Catalog, err error) {
		driver, ok := p[SqlDriverKey]
		if !ok {
			return nil, errors.New("must provide driver to pass to sql.Open")
		}

		dialect := strings.ToLower(p[SqlDialectKey])
		if dialect == "" {
			return nil, errors.New("must provide sql dialect to use")
		}

		uri = strings.TrimPrefix(p.Get("uri", uri), "sql://")
		...
		return NewSQLCatalog(p.Get("catalog.name", "sql"), sqldb, SupportedDialect(dialect), p)
	}))
}
...
var minimalNamespaceProps = iceberg.Properties{"exists": "true"}
...
// NewSQLCatalog creates a new sql-based catalog using the provided sql.DB handle to perform any queries.
//
...
// of catalog changes.
func NewSQLCatalog(name string, db *sql.DB, dialect SupportedDialect, props iceberg.Properties) (*SQLCatalog, error) {
	cat := &SQLCatalog{
		db:    bun.NewDB(db, getDialect(dialect)),
		name:  name,
		props: props,
	}
	...
	initTables, _ := strconv.ParseBool(props.Get(initCatalogTablesKey, "true"))
	if initTables {
...
func (c *SQLCatalog) CreateSQLTables(ctx context.Context) error {
...
func (c *SQLCatalog) DropSQLTables(ctx context.Context) error {
...
func (c *SQLCatalog) ensureTablesExist() error {
	var (
		ctx        = context.Background()
		mustCreate bool
		models     = []any{(*sqlIcebergTable)(nil), (*sqlIcebergNamespaceProps)(nil)}
	)

	var junk int
	for _, m := range models {
		if err := c.db.NewSelect().Model(m).ColumnExpr("1").Scan(ctx, &junk); err != nil {
			mustCreate = true
			break
		}
	}
	...
	return c.db.ResetModel(ctx, models...)
}

func (c *SQLCatalog) namespaceExists(ctx context.Context, ns string) (bool, error) {
...
func (c *SQLCatalog) getDefaultWarehouseLocation(ctx context.Context, nsname, tableName string) (string, error) {
...
func (c *SQLCatalog) resolveTableLocation(ctx context.Context, loc, nsname, tablename string) (string, error) {
	if len(loc) == 0 {
...
func (c *SQLCatalog) CreateTable(ctx context.Context, ident table.Identifier, sc *iceberg.Schema, opts ...createTableOpt) (*table.Table, error) {
	var cfg createTableCfg
	for _, opt := range opts {
		opt(&cfg)
	}

	nsIdent := NamespaceFromIdent(ident)
	tblIdent := TableNameFromIdent(ident)
	...
	if !exists {
		return nil, fmt.Errorf("%w: %s", ErrNoSuchNamespace, ns)
	}
	...
	if err := writeMetadata(metadata, metadataLocation, c.props); err != nil {
		return nil, err
	}
...
func (c *SQLCatalog) ListTables(ctx context.Context, namespace table.Identifier) ([]table.Identifier, error) {
	if len(namespace) > 0 {
		...
		if !exists {
			return nil, fmt.Errorf("%w: %s", ErrNoSuchNamespace, strings.Join(namespace, "."))
		}
	}
...
func (c *SQLCatalog) ListNamespaces(ctx context.Context, parent table.Identifier) ([]table.Identifier, error) {
	...
	if len(parent) > 0 {
		...
		if !exists {
			return nil, fmt.Errorf("%w: %s", ErrNoSuchNamespace, strings.Join(parent, "."))
		}
...
func (c *SQLCatalog) LoadTable(ctx context.Context, identifier table.Identifier, props iceberg.Properties) (*table.Table, error) {
	...
	iofs, err := iceio.LoadFS(tblProps, result.MetadataLocation.String)
	if err != nil {
		return nil, err
	}
...
func (c *SQLCatalog) DropTable(ctx context.Context, identifier table.Identifier) error {
	ns := strings.Join(NamespaceFromIdent(identifier), ".")
	tbl := TableNameFromIdent(identifier)
	...
	return withWriteTx(ctx, c.db, func(ctx context.Context, tx bun.Tx) error {
		...
		if n == 0 {
			return fmt.Errorf("%w: %s", ErrNoSuchTable, identifier)
		}
		return nil
	})
}
...
func (c *SQLCatalog) RenameTable(ctx context.Context, from, to table.Identifier) (*table.Table, error) {
	fromNs := strings.Join(NamespaceFromIdent(from), ".")
	fromTbl := TableNameFromIdent(from)

	toNs := strings.Join(NamespaceFromIdent(to), ".")
	toTbl := TableNameFromIdent(to)
	...
	if !exists {
		return nil, fmt.Errorf("%w: %s", ErrNoSuchNamespace, toNs)
	}
	...
	err = withWriteTx(ctx, c.db, func(ctx context.Context, tx bun.Tx) error {
		...
		if n == 0 {
			return fmt.Errorf("%w: %s", ErrNoSuchTable, from)
		}
		return nil
	})
	if err != nil {
...
func (c *SQLCatalog) CreateNamespace(ctx context.Context, namespace table.Identifier, props iceberg.Properties) error {
	...
	if exists {
		return fmt.Errorf("%w: %s", ErrNamespaceAlreadyExists, strings.Join(namespace, "."))
	}
...
func (c *SQLCatalog) DropNamespace(ctx context.Context, namespace table.Identifier) error {
	...
	if !exists {
		return fmt.Errorf("%w: %s", ErrNoSuchNamespace, nsToDelete)
	}
	...
	if len(tbls) > 0 {
		return fmt.Errorf("%w: %d tables exist in namespace %s", ErrNamespaceNotEmpty, len(tbls), nsToDelete)
	}
...
func (c *SQLCatalog) LoadNamespaceProperties(ctx context.Context, namespace table.Identifier) (iceberg.Properties, error) {
	...
	if !exists {
		return nil, fmt.Errorf("%w: %s", ErrNoSuchNamespace, nsToLoad)
	}
...
func (c *SQLCatalog) UpdateNamespaceProperties(ctx context.Context, namespace table.Identifier, removals []string, updates iceberg.Properties) (PropertiesUpdateSummary, error) {
