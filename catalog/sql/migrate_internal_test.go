...
func newInMemoryCatalog(t *testing.T, props iceberg.Properties) (*Catalog, *sql.DB) {
	...
	d, err := getDialect(SQLite)
	require.NoError(t, err)

	return &Catalog{db: bun.NewDB(sqldb, d), name: "test", props: props}, sqldb
}
...
func TestInitSchemaVersionV1WhenColumnPresent(t *testing.T) {
...
func TestInitSchemaVersionTableAbsentIsV1(t *testing.T) {
	...
	assert.Equal(t, schemaV1, c.schemaVersion)
	assert.False(t, c.isV0(), "a catalog whose iceberg_tables does not exist yet must be treated as fresh V1")
}

func TestInitSchemaVersionWithoutOptInStaysV0(t *testing.T) {
	c, sqldb := newInMemoryCatalog(t, nil)
	defer sqldb.Close()
...
func TestInitSchemaVersionWithOptInUpgradesToV1(t *testing.T) {
...
func TestInitSchemaVersionLowercaseOptInMigrates(t *testing.T) {
	c, sqldb := newInMemoryCatalog(t, iceberg.Properties{SchemaVersionKey: "v1"})
	defer sqldb.Close()

	createLegacyV0Table(t, sqldb)
	require.NoError(t, c.initSchemaVersion(context.Background()))

	assert.False(t, c.isV0(), "lowercase v1 opt-in must upgrade the catalog to V1 (case-insensitive, matching Java)")
	assert.True(t, icebergTypeColumnPresent(t, sqldb),
		"lowercase v1 opt-in must add the iceberg_type column")
}
...
func TestInitSchemaVersionProbeErrorFailsFast(t *testing.T) {
	sqldb, err := sql.Open(sqliteshim.ShimName, ":memory:")
	require.NoError(t, err)
	...
	c := &Catalog{db: bun.NewDB(sqldb, d), name: "test"}
	require.NoError(t, sqldb.Close())

	err = c.initSchemaVersion(context.Background())
	require.Error(t, err)
	assert.Contains(t, err.Error(), "detecting catalog schema version")
}

func TestIcebergTablesExistsQueryPerDialect(t *testing.T) {
	cases := []struct {
		name        dialect.Name
		mustContain []string
	}{
		{dialect.PG, []string{"information_schema.tables", "current_schema()", "iceberg_tables"}},
		{dialect.MySQL, []string{"information_schema.tables", "DATABASE()", "iceberg_tables"}},
		{dialect.MSSQL, []string{"information_schema.tables", "SCHEMA_NAME()", "iceberg_tables"}},
		{dialect.SQLite, []string{"sqlite_master", "iceberg_tables"}},
		{dialect.Oracle, []string{"user_tables", "ICEBERG_TABLES", "ROWNUM = 1"}},
	}
	for _, tc := range cases {
		t.Run(tc.name.String(), func(t *testing.T) {
			got, err := icebergTablesExistsQuery(tc.name)
			require.NoError(t, err)
			for _, sub := range tc.mustContain {
				assert.True(t, strings.Contains(got, sub),
					"table probe for %s missing %q: %s", tc.name, sub, got)
			}
		})
	}
}
